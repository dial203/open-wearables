...
Verify a user's connected accounts:
...
<Warning>
This returns one entry per **account**, not per provider. A user may hold
several accounts with the same provider — two Garmins worn at once for a device
comparison — so do not key a client on `provider` alone. See
...
[
  {
    ...
    "display_label": "P01 arm A",
    "account_index": 1,
    "account_count": 2,
    "observed_devices": ["fenix 7"],
    "created_at": "2025-01-15T10:30:00Z",
...
class OpenWearablesClient:
    ...
    async def get_connections(self, user_id: str) -> list[dict]:
        ...
        One entry per account, not per provider: a user may hold several with
        the same provider. Group on `id`, not `provider`.
        """
        async with httpx.AsyncClient() as client:
            resp = await client.get(
