...
|------------|-----------|------------|--------------|--------------------|
| `GOOGLE_USE_RECONCILE=true` | `dataPoints:reconcile` | native | **merged & deduplicated** | none |
| `GOOGLE_USE_RECONCILE=false` **(default in this fork)** | `dataPoints` (list) | native | raw, per-source | **per-device** |
...
```
    ...
    **Configuration details:**
    ...
    | `GOOGLE_USE_RECONCILE` | `true` = reconcile (merged, app-accurate, no device attribution); `false` = list (per-device). | `false` (this fork) |
