...
<Note>
**Two different things are called "multiple accounts". This page is one of them.**

- **This page** — *one* provider account shared by *several* OW profiles. One
  Garmin login, several people's profiles reading it. The problem is
  duplication: one payload, N profiles.
- **[Several accounts per provider](/dev-guides/multiple-provider-accounts)** —
  *one* OW profile holding *several* provider accounts. One participant wearing
  two Garmins, each on its own login. The problem is separation: N accounts
  whose data must never pool.

They are orthogonal and both apply at once: a profile holding two Garmin
accounts may share either of them with other profiles, and the coordination
below is keyed per external account, so each is handled independently.
</Note>
...
The page renders one card per *account*, not per provider, so a profile holding two Garmin accounts shows two cards and the chip applies to whichever of them is shared.
...
2. The activity or wellness data is **parsed once**, then saved independently for every connected *connection* within the same DB transaction (each insert is wrapped in a savepoint so a duplicate on one profile does not roll back the others). The unit is the connection rather than the profile: a profile holding two Garmin accounts has one connection per account, and only the one matching this `userId` receives the payload.
...
1. `try_become_primary(provider, provider_user_id, user_id, scope="pull")` is called.
   - **Won** → performs the API call normally; after completion triggers `sync_vendor_data` for all other linked profiles with `_skip_linked_fan_out=True` — and `connection_ids` naming the single connection that shares this external account, so a profile holding several accounts with the provider does not have the rest of them re-pulled as a side effect — so they receive the same date range without making their own API call; releases the lock.
...
| `app/integrations/celery/tasks/sync_vendor_data_task.py` | Shared pull lock per connection with `provider_user_id`; targets the fan-out at one connection via `connection_ids` |
| `app/utils/connection_context.py` | Binds each iteration of the per-account loop to one connection, so the provider calls behind it act for the right account |
